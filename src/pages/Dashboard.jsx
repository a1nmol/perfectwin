...
import {
  ...
  ChevronLeft, ChevronRight, MapPin, Layers, PanelRight, ArrowLeft
} from 'lucide-react';
...
function Dashboard() {
  ...
  const handleCloseProfile = () => setShowColonyProfile(false);

  const handleSearch = useCallback((query) => {
    setSearchQuery(query);
    if (query.length > 1 && coloniesData.length) {
      const q = query.toLowerCase();
      setSearchResults(coloniesData.filter(c => c.name.toLowerCase().includes(q)).slice(0, 8));
    } else {
      setSearchResults([]);
    }
  }, [coloniesData]);
    ...
    if (showColonyProfile && selectedColony) {
      return (
        <ColonyProfile
          ...
          onViewAI={() => { setShowColonyProfile(false); setActiveTab('ai-detection'); }}
        />
      );
    }
    return (
    ...
    return (
      <div className="h-[100dvh] bg-gray-950 flex items-center justify-center">
        <div className="text-center px-4">
          ...
          <p className="text-primary text-xl font-semibold">Loading EcoLens Dashboard...</p>
    ...
    <div className="h-[100dvh] flex flex-col bg-gray-950 overflow-hidden">
      ...
      <header className="h-14 flex items-center justify-between px-3 md:px-4 bg-gray-950 border-b border-gray-800 z-50 flex-shrink-0 gap-2 md:gap-4">
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          <Link to="/" className="flex items-center gap-1 px-2 py-1.5 rounded-lg hover:bg-gray-800 text-gray-500 hover:text-white transition-colors text-xs">
            ...
            <div className="absolute top-full left-0 right-0 mt-1 bg-gray-900 border border-gray-700 rounded-lg shadow-2xl z-[9999] overflow-hidden">
              {searchResults.map(colony => (
                <button
                  ...
                  onClick={() => { handleColonySelect(colony); setSearchQuery(''); setSearchResults([]); setMobileSearchOpen(false); }}
                >
            ...
            return (
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all whitespace-nowrap ${
                  isActive ? 'bg-primary/20 text-primary ring-1 ring-primary/30' : 'text-gray-500 hover:text-gray-200 hover:bg-gray-800/80'
                }`}
              >
      ...
      <div className="flex flex-1 overflow-hidden min-h-0">
          ...
          {tabs.map(tab => {
            ...
            return (
              ...
              >
                ...
                <span className="text-[9px] mt-1 leading-none font-medium tracking-wide">{tab.label.split(' ')[0]}</span>
              </button>
            );
          })}
        </nav>
        ...
        <div className="flex flex-col flex-1 overflow-hidden min-w-0">
          ...
          <div className="border-t border-gray-800 bg-gray-950 flex-shrink-0">
            <TimelineSlider coloniesData={coloniesData} selectedYear={selectedYear} onYearChange={setSelectedYear} />
          </div>
        </div>
          ...
          <div className="hidden md:flex w-[460px] flex-col border-l border-gray-800 bg-gray-950 flex-shrink-0 z-[900]">
            <div className="flex items-center justify-between px-3 py-2 border-b border-gray-800 flex-shrink-0 bg-gray-950">
              <span className="text-xs font-semibold text-gray-400 uppercase tracking-wider">
                {showColonyProfile && selectedColony ? selectedColony.name : tabs.find(t => t.id === activeTab)?.label ?? activeTab}
              </span>
              <button onClick={() => setRightPanelOpen(false)} className="flex items-center gap-1 px-2 py-1 rounded-md hover:bg-gray-800 text-gray-500 hover:text-gray-200 transition-colors text-xs">
      ...
      {/* ── Mobile bottom sheet ──────────────────────────────────── */}
      {rightPanelOpen && (
        <div className="md:hidden fixed inset-0 bg-black/50 z-[1900] backdrop-blur-sm" onClick={() => setRightPanelOpen(false)} />
      )}
      <div
